...
(async function() {
  ...
  exportCurrentBtn.addEventListener('click', async () => {
    ...
    // Regular chat URL: /chat/123456 (digits only after /chat/)
    const matchChat = pathname.match(/\/chat\/(\d+)(?![\w/])/);
    // Agent/Bot URL: /bot/123456 or /chat/bot/123456
    ...
    if (matchChat) {
        // Regular chat: conversation_id IS in the URL directly
        convId = matchChat[1];
        urlId = convId;
        const found = allConversations.find(c => String(c.id) === convId);
        if (found) convType = found.type || 0;

    } else if (matchBot) {
        // Agent/Bot chat: URL has bot_id, need to find real conversation_id
        const botId = matchBot[1];
        urlId = botId;
        isAgent = true;

        // Method 1: Check allConversations (if sidebar loaded recent list)
        const found = allConversations.find(c => c.botId === botId || String(c.botId) === String(botId));
        if (found) {
            convId = found.id;
            convType = found.type || 3;
        }

        // Method 2: Parse __NEXT_DATA__ (most reliable - always present for current session)
        if (!convId) {
            try {
                ...
                if (nextDataEl) {
                    ...
                    let foundType = 3;
                    function findConvIdInNext(obj) {
                        if (!obj || typeof obj !== 'object') return;
                        if (Array.isArray(obj)) {
                            for (let i = 0; i < obj.length; i++) { if (convId) return; findConvIdInNext(obj[i]); }
                            return;
                        }
                        if ((String(obj.bot_id) === String(botId) || String(obj.botId) === String(botId)) && obj.conversation_id) {
                            convId = String(obj.conversation_id);
                            if (obj.conversation_type) foundType = obj.conversation_type;
                            return;
                        }
                        for (const key of Object.keys(obj)) { if (convId) return; findConvIdInNext(obj[key]); }
                    }
                    findConvIdInNext(nextData);
                    if (convId) convType = foundType;
                }
            } catch(e) {}
        }

        // Method 3: Search all JSON script tags (React hydration data)
        if (!convId) {
            try {
                const scripts = document.querySelectorAll('script[type="application/json"]');
                for (const s of scripts) {
                    const text = s.textContent || '';
                    if (!text.includes('conversation_id')) continue;
                    try {
                        const data = JSON.parse(text);
                        function searchJson(obj) {
                            if (!obj || typeof obj !== 'object' || convId) return;
                            if (Array.isArray(obj)) { obj.forEach(searchJson); return; }
                            if ((String(obj.bot_id) === String(botId) || String(obj.botId) === String(botId)) && obj.conversation_id) {
                                convId = String(obj.conversation_id);
                                convType = obj.conversation_type || 3;
                                return;
                            }
    ...
    try {
        ...
        for (const sel of selectors) {
            ...
            if (el && el.innerText.trim()) { titleText = el.innerText.trim(); break; }
        }
        if (!titleText) {
            const activeItem = document.querySelector('[class*="session"][class*="active"]') ||
                               document.querySelector('[class*="item"][aria-selected="true"]');
            if (activeItem) titleText = activeItem.innerText.trim().split('\n')[0];
        }
    } catch(e) {}
    if (!titleText || titleText === '\u8c46\u5305') titleText = document.title.replace(' - \u8c46\u5305', '').trim();
    if (!titleText || titleText === '\u8c46\u5305') titleText = '\u5bf9\u8bdd_' + urlId;

    if (convId) {
        // Found real conversation_id - use full API export (complete history)
        const conv = {
            id: convId,
            title: titleText,
            type: convType,
        ...
        // Could not find conversation_id - offer DOM scraping fallback
        const doScrape = confirm(
            '\u65e0\u6cd5\u81ea\u52a8\u83b7\u53d6\u8be5\u667a\u80fd\u4f53\u7684\u5bf9\u8bdd ID\u3002\n\n' +
            '\u89e3\u51b3\u65b9\u6cd5\uff1a\n' +
            '1. \u5728\u5f53\u524d\u804a\u5929\u6846\u53d1\u9001\u4e00\u6761\u6d88\u606f\uff08\u5982\u201c\u4f60\u597d\u201d\uff09\uff0c\u5237\u65b0\u9875\u9762\u540e\u91cd\u8bd5\n' +
            '2. \u70b9\u201c\u786e\u5b9a\u201d\u5c1d\u8bd5\u6293\u53d6\u5f53\u524d\u9875\u9762\u53ef\u89c1\u5185\u5bb9\uff08\u4e0d\u5b8c\u6574\uff09\n\n' +
            '\u70b9\u201c\u786e\u5b9a\u201d\uff1a\u6293\u53d6\u5f53\u524d\u53ef\u89c1\u5185\u5bb9\u3000\u70b9\u201c\u53d6\u6d88\u201d\uff1a\u653e\u5f03'
        );
        if (!doScrape) return;

        const originalBtnHTML = exportCurrentBtn.innerHTML;
        exportCurrentBtn.innerHTML = '\u6b63\u5728\u6293\u53d6...';
        exportCurrentBtn.style.pointerEvents = 'none';
        exportCurrentBtn.style.opacity = '0.7';

        try {
            // Auto-scroll to load more
            const scroller = document.querySelector('[class*="scroller"][class*="v_list"]') ||
                             document.querySelector('.chat-scroller') ||
                             document.querySelector('[data-testid="chat-message-list"]');
            if (scroller) {
                let lastH = scroller.scrollHeight, same = 0;
                while (same < 4) {
                    scroller.scrollTop = 0;
                    scroller.dispatchEvent(new Event('scroll'));
                    await new Promise(r => setTimeout(r, 400));
                    if (scroller.scrollHeight === lastH) same++; else { same = 0; lastH = scroller.scrollHeight; }
                }
                await new Promise(r => setTimeout(r, 500));
            }

            const messages = [];
            const containers = document.querySelectorAll('[data-testid="message-block-container"]');
            if (containers.length > 0) {
                containers.forEach(c => {
                    const u = c.querySelector('[data-testid="send_message"]');
                    if (u && u.innerText.trim()) { messages.push({ isUser: true, text: u.innerText.trim() }); return; }
                    const a = c.querySelector('[data-testid="receive_message"]');
                    if (a && a.innerText.trim()) messages.push({ isUser: false, text: a.innerText.trim() });
                });
            } else {
                document.querySelectorAll('[class*="v_list_row"]').forEach(row => {
                    const el = row.querySelector('[data-message-id]') || row.querySelector('[class*="message-content"]');
                    if (!el || !el.innerText.trim()) return;
                    const isUser = !!row.querySelector('[data-testid="chat-message-action-edit"]') ||
                                   !!row.querySelector('.user-avatar') || row.className.includes('user');
                    messages.push({ isUser, text: el.innerText.trim() });
                });
            }

            if (messages.length === 0) {
                alert('\u9875\u9762\u6293\u53d6\u5931\u8d25\uff0c\u672a\u627e\u5230\u6d88\u606f\u5143\u7d20\u3002\u5efa\u8bae\u5148\u53d1\u9001\u4e00\u6761\u6d88\u606f\u518d\u91cd\u8bd5\u3002');
                return;
            }

            let md = '# ' + titleText + ' (\u9875\u9762\u6293\u53d6\u7248)\n\n> \u26a0\ufe0f \u6b64\u5185\u5bb9\u7531\u9875\u9762\u76f4\u63a5\u6293\u53d6\uff0c\u53ef\u80fd\u4e0d\u5b8c\u6574\u3002\n\n';
            messages.forEach(m => { md += '**' + (m.isUser ? '\u6211' : titleText) + '**\uff1a\n' + m.text + '\n\n'; });

            const blob = new Blob([md], { type: 'text/markdown;charset=utf-8' });
            const a = document.createElement('a');
            a.href = URL.createObjectURL(blob);
            a.download = titleText + '.md';
            a.click();
